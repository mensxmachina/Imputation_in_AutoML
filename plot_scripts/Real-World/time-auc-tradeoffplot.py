...
time_df = pd.read_csv('Time-Execution-Results.csv',sep=';',engine='python',na_values='?')
Auc_df = pd.read_csv('Final-Real-World-Results.csv',sep=';',engine='python',na_values='?')
...
plt.fill_between([0.7, 1],1,60000,alpha=0.3, color='#00CC00')  # blue
plt.fill_between([1, 1.2], -10000,1, alpha=0.3, color='#CC0000')  # yellow
plt.fill_between([0.7, 1], -10000,1, alpha=0.3, color='#C0C0C0')  # orange
plt.fill_between([1, 1.2], 1,60000, alpha=0.3, color='#C0C0C0')  # red

plt.text(0.8,35000, 'BI+MM Domination', style='italic')
plt.text(0.8,30000,str(better_faster)+' points', style='italic')
plt.text(1.05,30000, str(worse_faster)+' points', style='italic')
...
for i in res_noft_and_Ft_auc_ready.columns:
    ...
    plt.scatter(res_noft_and_Ft_auc_ready[i].mean(),res_Overall_time_ready[i].mean(),marker= marker_sele,s=SCATTER_MEDIAN,color=color,zorder=2,edgecolors='black',alpha=0.8)
    plt.scatter(res_noft_and_Ft_auc_ready[i],res_Overall_time_ready[i],marker= marker_sele,label=i,s=SCATTER_BIG,color=color)
plt.legend(loc='upper right')
plt.yticks([60000,50000,40000,30000,20000,10000,1.0,-10000],['6x$10^4$','5x$10^4$','4x$10^4$','3x$10^4$','2x$10^4$','$10^4$','1.0','-$10^4$'])
...
for i in res_noft_and_Ft_auc_ready.columns:
    ...
    linestl,color,smth=color_symbol_marker_column_match(i)
    print(res_noft_and_Ft_auc_ready[i].mean(skipna=True),res_Overall_time_ready[i].mean(skipna=True),i)
    plt.scatter(res_noft_and_Ft_auc_ready[i].mean(),res_Overall_time_ready[i].mean(),marker= marker_sele,s=SCATTER_MEDIAN,color=color,zorder=2,edgecolors='black',alpha=1)
    plt.scatter(res_noft_and_Ft_auc_ready[i],res_Overall_time_ready[i],marker= marker_sele,label=i,s=SCATTER_BIG,color=color,zorder=1,alpha = 0.6)
