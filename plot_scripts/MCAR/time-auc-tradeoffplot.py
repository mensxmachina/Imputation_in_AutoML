...
time_df = pd.read_csv('Mcar_50-Time-Execution-Results.csv',sep=';',na_values='?',float_precision='high')
Auc_df = pd.read_csv('MCAR_Holdout_Results.csv',sep=';',na_values='?',float_precision='high')
...
for i in res_noft_and_Ft_auc_ready:
    for key,values in res_noft_and_Ft_auc_ready[i].items():
        print(values,res_Overall_time_ready[i].loc[key])
...
print(res_noft_and_Ft_auc_ready.max(),res_Overall_time_ready.max())
print(res_noft_and_Ft_auc_ready.min(),res_Overall_time_ready.min())
mng = plt.get_current_fig_manager()
mng.full_screen_toggle()
plt.show()
