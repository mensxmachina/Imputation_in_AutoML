...
import sys
sys.path.append('../')
from Plot_parameters import colors,medianprops_diff,medianprops_normal,meanprops_normal,SMALL_SIZE,MEDIUM_SIZE,BIGGER_SIZE,X_AXIS_SIZE,Y_AXIS_SIZE,LEGEND_SIZE,ULTRA_SIZE,SCATTER_SMALL,SCATTER_BIG,LINE_WIDTH,MARKER_SIZE_SMALL,MARKER_SIZE_BIG
...
res_no_Ft_tradeoff.to_csv('MCAR-From-Baseline-No-FS.csv')"""
"""

fig, ax = plt.subplots()
plt.title('AUC difference from complete dataset in MCAR simulation when feature selection is excluded')
boxplots = list()
j=0
i=0
res_no_Ft_tradeoff.reset_index(inplace=True)
res_no_Ft = res_no_Ft_tradeoff[['Best','MM','BI+MM','MF','BI+MF','GAIN','BI+GAIN','SOFT','BI+SOFT','PPCA','BI+PPCA','DAE','BI+DAE']]
...
plt.ylabel('AUC_Difference = AUC of each imputor - AUC of MM ')
ax.legend([boxplots[i]['boxes'][0] for i in range(len(boxplots))],[i for i in res_no_Ft.columns], loc='lower right')
plt.rcParams["savefig.bbox"] = "tight"
#plt.show()
mng = plt.get_current_fig_manager()
mng.full_screen_toggle()
plt.savefig('plots/MCAR_Difference_From_baseline_without_fs.png',bbox_inches='tight')
plt.close()"""
...
res_Ft_tradeoff.to_csv('MCAR-From-Baseline-with-FS.csv')"""

"""
fig, ax = plt.subplots()
plt.title('AUC difference from complete dataset in MCAR simulation when feature selection is enforced')
boxplots = list()
j=0
i=0
#['MM','BI+MM','GAIN','BI+GAIN','MF','BI+MF','SOFT','BI+SOFT','PPCA','BI+PPCA']
res_Ft_tradeoff.reset_index(inplace=True)
res_Ft = res_Ft_tradeoff[['Best','MM','BI+MM','MF','BI+MF','GAIN','BI+GAIN','SOFT','BI+SOFT','PPCA','BI+PPCA','DAE','BI+DAE']]
...
mng = plt.get_current_fig_manager()
mng.full_screen_toggle()
plt.savefig('plots/MCAR_Difference_From_baseline_with_fs.png',bbox_inches='tight')
plt.close()"""
...
plt.title('Average AUC difference from basic imputation method in MAR simulation')
#['MM','BI+MM','GAIN','BI+GAIN','MF','BI+MF','SOFT','BI+SOFT','PPCA','BI+PPCA']
...
for pair in list_pairs_imp:
    ...
    res_overall_tradeoff[with_bi] = res_overall_tradeoff[with_bi].subtract(res_overall_tradeoff[without_bi])

best_overall = res_overall_tradeoff[['BI+MF','BI+SOFT','BI+MM','BI+PPCA','BI+DAE','BI+GAIN']] #['Best','MM','MF','GAIN','SOFT','PPCA','DAE']
print(best_overall)
legend_elements = []
for i in best_overall.columns:
    dt = [np.mean(best_overall.loc[list_10][i]),np.mean(best_overall.loc[list_25][i]),np.mean(best_overall.loc[list_50][i])]
    line_style,color,marker=color_symbol_marker_column_match(i)
    plt.scatter([0,1,2],dt,c=color,marker=marker,s=SCATTER_BIG,zorder=2)
    plt.plot(dt,linestyle=line_style,color = color,linewidth = LINE_WIDTH,zorder=1)
...
#plt.legend([i for i in best_overall.columns],loc='upper right')
#plt.show()
...
mng = plt.get_current_fig_manager()
mng.full_screen_toggle()
plt.savefig('plots/MAR_Diff_Indi_vs_No_Indi.png',bbox_inches='tight')
plt.close()
